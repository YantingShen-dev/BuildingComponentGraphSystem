...
if __name__ == "__main__":
    ...
    # 获取data目录下的所有子文件夹并筛选421-999范围内的文件夹
    data_folders = [f for f in os.listdir(BASE_DATA_DIR) 
                   if os.path.isdir(os.path.join(BASE_DATA_DIR, f)) and 
                   f.isdigit()]
    data_folders.sort(key=int)  # 按数字大小排序
    num_datasets = len(data_folders)
    print(f"Found {num_datasets} datasets between 421-999 in {BASE_DATA_DIR}")
    ...
    # 循环处理每个数据集
    for folder in sorted(data_folders):  # 排序以确保按顺序处理
        print(f"Processing dataset {folder}...")
        data_dir = os.path.join(BASE_DATA_DIR, folder)
        
        # 为每个数据集创建对应的输出目录
        dataset_output_dir = os.path.join(OUTPUT_DIR, folder)
        os.makedirs(dataset_output_dir, exist_ok=True)
        
        try:
            # 创建解释器实例
            explainer = GCNExplainer(MODEL_PATH, data_dir)
            ...
            print(f"Error processing dataset {folder}: {str(e)}")
            continue
