  ...
  <div class="home">
    ...
    <div class="viewport-container">
      ...
      <div class="export-button-container">
        <button @click="goToOptView" class="export-button">
          {{ currentLanguage === 'zh' ? '优化分析' : 'Optimization Analysis' }}
        </button>
      </div>
      ...
      <div class="notch-container">
        <div class="notch">
          <span class="notch-text">
            {{ currentLanguage === 'zh' ? '第二步：模型分析' : 'Step 2: Model Analysis' }}
          </span>
        </div>
      </div>
    </div>
    <div class="right-container">
      <div class="right-panel upper-panel">
        ...
        <div class="notch-container">
          <div class="notch">
            <span class="notch-text">
              {{ currentLanguage === 'zh' ? '模型分析结果' : 'Model Analysis Results' }}
            </span>
          </div>
        </div>
        <div class="panel-content">
          <div v-if="apiResponse" class="api-response">
            ...
            <div class="result-section prediction-section">
              <h3>
                {{ currentLanguage === 'zh' ? '预测能耗' : 'Predicted Energy Consumption' }}
              </h3>
              <div class="prediction-value">{{ apiResponse.prediction ? apiResponse.prediction.toFixed(2) : '未知' }} kWh</div>
            </div>

            <!-- 节点重要性表格 -->
            <div class="result-section" v-if="apiResponse.explanation && apiResponse.explanation.node_importance">
              <h3>
                {{ currentLanguage === 'zh' ? '节点（构件）重要性分析' : 'Node (Component) Importance Analysis' }}
              </h3>
              <div class="table-container">
                <table class="data-table">
                  <thead>
                    <tr>
                      <th>
                        {{ currentLanguage === 'zh' ? '节点' : 'Node' }}
                      </th>
                      <th v-for="feature in orderedFeatures" :key="'feat-'+feature">
                        {{ getDisplayName(feature) }}
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr v-for="(nodeId, index) in apiResponse.explanation.node_importance.Node_ID" :key="'node-'+index"
                        @click="highlightGroup(index)" class="clickable-row">
                      <td>
                        {{ currentLanguage === 'zh' ? '节点' : 'Node' }} {{ index }}
                      </td>
                      <td v-for="feature in orderedFeatures" :key="'feat-'+feature"
                          :style="getCellStyle(apiResponse.explanation.node_importance[feature][index])">
                        {{ parseFloat(apiResponse.explanation.node_importance[feature][index]).toFixed(2) }}
                      </td>
                    </tr>
                  </tbody>
                </table>
            ...
            <!-- 边重要性矩阵 -->
            <div class="result-section" v-if="apiResponse.explanation && apiResponse.explanation.edge_matrix">
              <h3>
                {{ currentLanguage === 'zh' ? '链接重要性分析' : 'Connection Importance Analysis' }}
              </h3>
              <div class="table-container">
                <table class="data-table">
                  <thead>
                    <tr>
                      <th></th>
                      <th v-for="(_, index) in apiResponse.explanation.edge_matrix" :key="'edge-col-'+index">
                        {{ currentLanguage === 'zh' ? '节点' : 'Node' }} {{ index }}
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr v-for="(row, rowIndex) in apiResponse.explanation.edge_matrix" :key="'edge-row-'+rowIndex">
                      <td>
                        {{ currentLanguage === 'zh' ? '节点' : 'Node' }} {{ rowIndex }}
                      </td>
                      <td v-for="(value, colIndex) in row" :key="'edge-cell-'+rowIndex+'-'+colIndex"
            ...
            <!-- 添加Top10绝对值最大的值 -->
              ...
              <div class="top-values-container">
                ...
                <div class="top-values-section">
                  <h4>
                    {{ currentLanguage === 'zh' ? '节点(构件)重要性前十' : 'Top 10 Node (Component) Importance' }}
                  </h4>
                  <!-- 添加直方图 -->
                  <div class="chart-container">
                    <div class="bar-chart">
                      <div v-for="(item, index) in getTopNodeImportance()" :key="'node-chart-'+index"
                          class="bar-item" @click="highlightGroup(item.nodeIndex)">
                        <div class="bar-label">{{ getDisplayName(item.feature) }} ({{ currentLanguage === 'zh' ? '节点' : 'Node' }} {{ item.nodeIndex }})</div>
                        <div :class="['bar', item.value > 0 ? 'bar-positive' : 'bar-negative']"
                             :style="{
                               width: getBarWidth(item.value, getTopNodeImportance()),
                               backgroundColor: item.value > 0 ? 'rgba(255, 0, 0, 0.5)' : 'rgba(0, 128, 0, 0.5)'
                             }">
                        </div>
                        <span class="bar-value" :style="{ [item.value > 0 ? 'left' : 'right']: 'calc(50% + ' + getBarWidth(item.value, getTopNodeImportance()) + ')' }">
                ...
                <div class="top-values-section">
                  <h4>
                    {{ currentLanguage === 'zh' ? '链接重要性前十' : 'Top 10 Connection Importance' }}
                  </h4>
                  <!-- 添加直方图 -->
                  <div class="chart-container">
                    <div class="bar-chart">
                      <div v-for="(item, index) in getTopEdgeImportance()" :key="'edge-chart-'+index"
                          class="bar-item">
                        <div class="bar-label">
                          {{ currentLanguage === 'zh' ? '节点' : 'Node' }} {{ item.source }} → {{ item.target }}
                        </div>
                        <div :class="['bar', item.value > 0 ? 'bar-positive' : 'bar-negative']"
                             :style="{
                               width: getEdgeBarWidth(item.value, getTopEdgeImportance()),
                               backgroundColor: item.value > 0 ? 'rgba(255, 0, 0, 0.5)' : 'rgba(0, 128, 0, 0.5)'
                             }">
                        </div>
                        <span class="bar-value" :style="{ [item.value > 0 ? 'left' : 'right']: 'calc(50% + ' + getEdgeBarWidth(item.value, getTopEdgeImportance()) + ')' }">
                          {{ parseFloat(String(item.value)).toFixed(2) }}
                        </span>
                      </div>
                    </div>
  ...
  <!-- <div v-if="showWindowInfoTooltip && selectedWindowInfo"
    ...
    <div class="window-info-content">
      <div class="window-info-item">
        <span class="window-info-label">类型:</span>
      ...
      <div class="window-info-item">
        <span class="window-info-label">长度:</span>
        <input
          type="number"
      ...
      <div class="window-info-item">
        <span class="window-info-label">宽度:</span>
        <input
          type="number"
          class="window-info-input"
      ...
      <div class="window-info-item">
        <span class="window-info-label">SHGC值:</span>
        <input
      ...
      <div class="window-info-action">
        <button class="window-delete-button" @click="deleteSelectedWindow">删除</button>
      </div>
    </div>
  </div> -->
...
const handleBackButton = () => {
  ...
  if (window.confirm(currentLanguage.value === 'zh' ? '确定返回上一步吗？' : 'Are you sure you want to go back?')) {
    ...
    console.log('返回SecondView前已保存面组数据');

    // 清理localStorage中的API数据，因为要返回到SecondView
    localStorage.removeItem('thirdViewApiResponse');
    // 用户确认后，导航到SecondView页面
    router.push('/second');
  }
};
...
// 跳转到优化分析页面
const goToOptView = () => {
  // 确保API数据已保存到localStorage
  if (apiResponse.value) {
    localStorage.setItem('thirdViewApiResponse', JSON.stringify(apiResponse.value));
    // 将API数据作为路由参数传递到OptView
    router.push({
      path: '/opt',
      query: { response: JSON.stringify(apiResponse.value) }
    });
  } else {
    // 如果没有API数据，仍然跳转但不传递数据
    router.push('/opt');
  }
};
...
onMounted(() => {
  ...
  // 在组件挂载时从路由参数或localStorage获取API返回数据
  if (route.query.response) {
    try {
      apiResponse.value = JSON.parse(route.query.response as string);
      console.log('从路由参数解析到API返回数据:', apiResponse.value);
      // 保存到localStorage以便后续使用
      localStorage.setItem('thirdViewApiResponse', route.query.response as string);
    } catch (error) {
    ...
    // 尝试从localStorage恢复数据
    const savedApiResponse = localStorage.getItem('thirdViewApiResponse');
    if (savedApiResponse) {
      try {
        apiResponse.value = JSON.parse(savedApiResponse);
        console.log('从localStorage恢复API返回数据:', apiResponse.value);
      } catch (error) {
        console.error('从localStorage恢复API数据失败:', error);
        apiResponse.value = { error: '恢复API数据失败' };
      }
    } else {
...
// 根据数值计算单元格背景色
const getCellStyle = (value: string | number) => {
  // 将value转换为数字类型
  const numValue = typeof value === 'string' ? parseFloat(value) : value;

  if (numValue === 0) return { backgroundColor: '#ffffff' }; // 白色

  if (numValue > 0) {
    // 正数：红色渐变 - 值越大颜色越深
    const opacity = Math.min(Math.abs(numValue), 1); // 限制最大为1
    return { backgroundColor: `rgba(255, 0, 0, ${opacity * 0.5})` };
  } else {
    // 负数：绿色渐变 - 值越小（绝对值越大）颜色越深
    const opacity = Math.min(Math.abs(numValue), 1); // 限制最大为1
    return { backgroundColor: `rgba(0, 128, 0, ${opacity * 0.5})` };
  }
};

// 获取节点重要性前十的值
const getTopNodeImportance = () => {
  ...
  const features = Object.keys(nodeImportance).filter(key => key !== 'Node_ID');
  const topValues: {nodeIndex: number; feature: string; value: number}[] = [];

  // 遍历所有特征和节点
  features.forEach(feature => {
    Object.keys(nodeImportance[feature]).forEach(nodeId => {
      const value = nodeImportance[feature][nodeId];
      if (value !== 0) { // 跳过0值
        topValues.push({
          nodeIndex: parseInt(nodeId),
          feature,
          value: Number(value)
        });
      }
  ...
  // 按绝对值大小排序
  topValues.sort((a, b) => Math.abs(b.value) - Math.abs(a.value));

  // 返回前十个
  return topValues.slice(0, 10);
};

// 获取边重要性前十的值
const getTopEdgeImportance = () => {
  if (!apiResponse.value?.explanation?.edge_matrix) return [];

  const edgeMatrix = apiResponse.value.explanation.edge_matrix;
  const topValues: {source: number; target: number; value: number}[] = [];

  // 遍历上三角矩阵（避免重复）
  for (let i = 0; i < edgeMatrix.length; i++) {
    for (let j = i; j < edgeMatrix[i].length; j++) {
      const value = edgeMatrix[i][j];
      if (value !== 0) { // 跳过0值
        topValues.push({
          source: i,
          target: j,
...
// 计算节点重要性直方图宽度
const getBarWidth = (value: number, topValues: {nodeIndex: number; feature: string; value: number}[]): string => {
  const maxValue = Math.max(...topValues.map(item => Math.abs(item.value)));
  const barWidth = Math.min(Math.abs(value) / maxValue * 100, 100);
  return `${barWidth * 0.4}%`;
};

// 计算链接重要性直方图宽度
const getEdgeBarWidth = (value: number, topValues: {source: number; target: number; value: number}[]): string => {
  const maxValue = Math.max(...topValues.map(item => Math.abs(item.value)));
  const barWidth = Math.min(Math.abs(value) / maxValue * 100, 100);
  return `${barWidth * 0.4}%`;
};
